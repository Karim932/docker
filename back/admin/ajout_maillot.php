<?php
    ...
    if (!isset($_SESSION['pseudo'])) {
        header('Location: /var/www/html/connexion.php');
        exit;
    }else if ($_SESSION['admin'] != 1){
        header("location: /var/www/html/front/index.php");
        exit;
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (isset($_FILES['image_maillot']) && $_FILES['image_maillot']['error'] === UPLOAD_ERR_OK) {
            $uploadDir = '/var/www/html/maillot/';
            $uploadFile = $uploadDir . basename($_FILES['image_maillot']['name']);
            $uploadDirBack = '../maillot/';
            $uploadFileBack = $uploadDirBack . basename($_FILES['image_maillot']['name']);

            if (move_uploaded_file($_FILES['image_maillot']['tmp_name'], $uploadFile)) {
                $cheminImage = $uploadFileBack;
